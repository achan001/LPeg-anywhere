...
/*
// number of characters to match a pattern (or INT32_MIN if variable)
*/
int fixedlen (TTree *tree) {
 ...
 tailcall:
  switch (tree->tag) {
    case TChar: case TSet: case TAny:
      return len + 1;
    case TBehind: if (sib1(tree)->tag == TNot) return len - (tree->u.n);
      // fall thru for lpeg.B(n)
    case TFalse: case TTrue: case TNot: case TAnd:
      return len;
    ...
    case TCapture: case TRule: case TGrammar:
      ...
      tree = sib1(tree); goto tailcall;
    case TCall: {
      int n1 = callrecursive(tree, fixedlen, INT32_MIN);
      if (n1 == INT32_MIN)
        return INT32_MIN;
      else
        return len + n1;
...
static void coderep (CompileState *compst, TTree *tree, int opt,
  ...
  if (tocharset(tree, &st)) {
    int c = (tree->tag != TSet);
    if (c == 0) loopset(i, c |= st.cs[i]);
    if (c == 0) return;     // skip useless ISpan[]
