...
tailcall:
  switch (tree->tag) {
    case TChar: 
      setchar(treebuffer(set), tree->u.n); break;
    case TAny: case TNot: case TBehind:
      loopset(i, treebuffer(set)[i] = 0xff); break;
    case TSet: 
      loopset(i, treebuffer(set)[i] |= treebuffer(tree)[i]); break;
    case TAnd: case TRep: case TCapture: case TRunTime:
      tree = sib1(tree); goto tailcall;
    case TChoice: case TSeq:
      switch (head_chars(sib1(tree), set)) {
        case -1: return -1;     // error, ignore sib2
        case  1: if (tree->tag == TSeq) return 1;
        default: tree = sib2(tree); goto tailcall;
      }
    case TTrue: return 0;       // lpeg.Cp ?
    default: return -1;         // bad tree->tag
  }
  return 1;
}
...
static int lp_head(lua_State *L) {
  ...
  if (head_chars(tree, set) < 0) luaL_error(L, "cannot build head chars");
  lua_remove(L, 1);
  return 1;
}
