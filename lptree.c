...
tailcall:
  switch (tree->tag) {
    case TChar: 
      setchar(treebuffer(set), tree->u.n); return 2;
    case TAny: case TNot: case TBehind:
      loopset(i, treebuffer(set)[i] = 0xff); return 2;
    case TSet: 
      loopset(i, treebuffer(set)[i] |= treebuffer(tree)[i]); return 2;
    case TAnd: case TRep: case TCapture: case TRunTime:
      tree = sib1(tree); goto tailcall;
    case TChoice: case TSeq:
      switch (head_chars(sib1(tree), set)) {
        case 0: return 0;       // error, ignore sib2
        case 2: if (tree->tag == TSeq) return 2;
        default: tree = sib2(tree); goto tailcall;
      }
    case TTrue: return 1;       // P'' or Cp()
    default: return 0;          // bad tree->tag
  }
}

static int lp_head(lua_State *L) {
  TTree *tree = getpatt(L, 1, NULL);
  TTree *set = newcharset(L);   // empty set
  if (!head_chars(tree, set)) luaL_error(L, "cannot build head chars");
  return 1;
}
