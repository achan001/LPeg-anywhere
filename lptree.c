...
static int lp_skip(lua_State *L) {
  ...
  TTree *skip = newcharset(L);
  if (head_chars(tree, skip) == 3) {        // skip = ~ head-chars
    loopset(i, treebuffer(skip)[i] = ~treebuffer(skip)[i]);
  } else
    loopset(i, treebuffer(skip)[i] = 0);    // be safe, skip = set[]
  return 1;
}
