...
static int lp_star (lua_State *L) {
  ...
  if (n >= 0) {  /* seq tree1 (seq tree1 ... (seq tree1 (rep tree1))) */
    int pow = lua_toboolean(L, 3);  // true pow = no rep code
    if ((n -= pow) < 0) {newleaf(L, TTrue); return 1;}
    tree = newtree(L, (n + 1) * (size1 + 1) - pow);
    if (!pow && nullable(tree1))
      luaL_error(L, "loop body may accept empty string");
    while (n--)  /* repeat 'n' times */
      tree = seqaux(tree, tree1, size1);
    if (!pow) {tree->tag = TRep; tree = sib1(tree);}
  }
    ...
    for (; n > 1; n--) {  /* repeat (n - 1) times */
      ...
      sib2(tree)->tag = TTrue;
      tree->tag = TChoice; tree = sib1(tree);
      tree = seqaux(tree, tree1, size1);
    }
    tree->u.ps = size1 + 1;
    sib2(tree)->tag = TTrue;
  ...
  memcpy(tree, tree1, size1 * sizeof(TTree));
  copyktable(L, 1);
  return 1;
}
...
static int head_chars(TTree *tree, TTree *set)
{
tailcall:
  switch (tree->tag) {
    case TAny: case TNot: case TBehind: return 2;
    case TChar: setchar(treebuffer(set), tree->u.n); return 3;
    case TSet:
      loopset(i, treebuffer(set)[i] |= treebuffer(tree)[i]); return 3;
    case TAnd: case TRep: case TCapture: case TRunTime:
      tree = sib1(tree); goto tailcall;
    case TChoice: case TSeq:
      switch (head_chars(sib1(tree), set)) {
        case 1:                 // bad tree->tag
        case 2: return 1;       // full set of head-chars
        case 3: if (tree->tag == TSeq) return 3;
        default: tree = sib2(tree); goto tailcall;
      }
    case TTrue: return 0;       // to skip Cp()
    default: return 1;
  }
}

static int lp_skip(lua_State *L) {
  TTree *tree = getpatt(L, 1, NULL);
  TTree *set = newcharset(L);           // lua_toboolean default=true
  byte h = lua_toboolean(L, 2) - 1;     // true=skip, false=head-chars
  if (head_chars(tree, set) < 3) {      // bad input, so be safe
    loopset(i, treebuffer(set)[i] = h); // skip=Set[] -> head=any
  } else if (!h)                        // skip = ~head-chars
    loopset(i, treebuffer(set)[i] = ~treebuffer(set)[i]);
  return 1;
}
...
static int lp_behind (lua_State *L) {
  TTree *tree  = getpatt(L, 1, NULL);   // skip 1 sign
  TTree *tree1 = tree->tag == TNot ? sib1(tree) : tree;
...
static int lp_divcapture (lua_State *L) {
  switch (lua_type(L, 2)) {
    ...
    case LUA_TNUMBER: {
      ...
      TTree *tree = newroot1sib(L, TCapture);
      tree->cap = Cnum;
      tree->key = (short) n;    // -1 == last capture
      luaL_argcheck(L, n == (short) tree->key, 1, "invalid number");
      return 1;
    }
